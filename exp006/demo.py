...
class Demo(BaseDemo):
    ...
    def train(self):
        ...
        for epoch in range(self.train_epoch):
            ...
            im_pred = F.relu(im_pred)
            im_pred = 1 - F.relu(1 - im_pred)
            loss = F.binary_cross_entropy(im_pred, im_input[:, -self.im_channel:, :, :], size_average=False)
            loss.backward()
            optimizer.step()
            ...
            base_loss = F.binary_cross_entropy(im_output, im_input[:, -self.im_channel:, :, :], size_average=False)
    ...
    def test(self):
        ...
        for epoch in range(self.test_epoch):
            ...
            loss = loss_func(im_pred, im_input[:, -self.im_channel:, :, :])

            test_loss_all.append(loss.data[0])
            base_loss = loss_func(im_output, im_input[:, -self.im_channel:, :, :])
            base_loss_all.append(base_loss.data[0])
